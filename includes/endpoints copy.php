...
class endpoints
{
    ...
    static function entried_event($request)
    {
        ...
        if ($line_user_query->have_posts()) {
            ...
            if (isset($entry_history) && !empty($entry_history)) {
                ...
                // 申込内容（設問ごとに別metaで保存）
                $selected = [];
                $v1 = get_post_meta($entry_history_id, 'event_types', true);
                $v2 = get_post_meta($entry_history_id, 'event_type2_answers', true);
                $v3 = get_post_meta($entry_history_id, 'event_type3_answers', true);
                $v4 = get_post_meta($entry_history_id, 'event_type4_answers', true);
                foreach ([$v1, $v2, $v3, $v4] as $v) {
                    $v = is_string($v) ? trim($v) : '';
                    if ($v !== '') $selected[] = $v;
                }
                $event_types = '';
                foreach ($selected as $event_type) {
                    $event_types .= '<li>' . esc_html($event_type) . '</li>' . PHP_EOL;
                }
            }
    ...
    static function event_list()
    {
        ...
        while ($event_query->have_posts()) {
            ...
            $thirty_days_later->modify('+60 days');
    ...
    static function get_mypage($request)
    {
        ...
        if ($line_user_query->have_posts()) {
            ...
            $html .= '    <li><a href="https://liff.line.me/' . $liff_id_profile . '">登録情報</a></li>' . PHP_EOL;
