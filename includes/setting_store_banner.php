...
class settingStoreBanner
{

    const LABEL = '店舗広告';
    const POST_TYPE = 'store_banner';
     ...
     */
    static function set_store_banner_post_type()
    {
        $label = self::LABEL;
     ...
     * @return void 
     */
    static function create_store_banner_custom_fields()
    {
        $label = self::LABEL;
        $post_type = self::POST_TYPE;
        $fields = self::$fields;
        ...
        foreach ($fields as $value => $name) {
            add_meta_box(
                $value, //編集画面セクションID
                $name, //編集画面セクションのタイトル
                ['settingStoreBanner', 'show_' . $value], //編集画面セクションにHTML出力する関数
                $post_type, //投稿タイプ名
...
?>
    ...
    static function show_store_detail($post)
    {
        $item_name = 'store_detail';
        $post_id = $post->ID;
        $value = get_post_meta($post_id, $item_name, true);
?>
<textarea name="<?= $item_name; ?>" id="<?= $item_name; ?>"><?= esc_attr($value); ?></textarea>

    <?php
    }

    static function show_store_url($post)
    {
        $item_name = 'store_url';
        $post_id = $post->ID;
        $value = get_post_meta($post_id, $item_name, true);
