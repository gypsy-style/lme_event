...
class lineconnectRichmenu{
    ...
    static function updateRichMenu($userId,$richMenuId)
    {
        // LINE BOT SDK
    ...
    リッチメニューIDの更新
    */
    static function updateRichMenuId($state, $richmenu_id){
        // LINE BOT SDK
        require_once(plugin_dir_path(__FILE__).'../vendor/autoload.php');
        ...
        if($state == 'linked'){
            ...
            $args = array(
                'meta_query' => array(
                    array(
                        'key'     => lineMembers::META_KEY__LINE,
                        'compare' => 'EXISTS'
                    )
                ),
                //'role' => $role,
            ...
            if(! empty( $users )){	//マッチするユーザーが見つかれば
                ...
                foreach($users as $user){
                    $user_meta_line = $user->get(lineMembers::META_KEY__LINE);
